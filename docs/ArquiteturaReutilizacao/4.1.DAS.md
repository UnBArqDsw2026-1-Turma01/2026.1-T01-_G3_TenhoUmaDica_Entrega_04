...
Os casos de uso estão organizados em seis módulos funcionais identificados no fluxograma do sistema TenhoUmaDica. Ela consolida as ações primárias e secundárias que os atores Estudante e Moderador podem executar na plataforma, definindo o escopo total das funcionalidades que a aplicação deve suportar.
...
Aqui está a versão limpa e revisada. O texto foi simplificado para eliminar palavras excessivamente formais ou complexas (como "preconizado", "subsídio", "ecossistema", "imaculadas" e "ambas"), tornando a leitura mais direta e natural para o contexto acadêmico.
...
A estrutura do servidor é organizada por pacotes de domínio funcional, utilizando o NestJS como framework para gerenciar os módulos e as dependências da aplicação. As regras de domínio e atributos descritos aqui servem de base para o **Diagrama de Classes**.

### 5.1.1 Núcleo da Aplicação (Composição e Bootstrap)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `main.ts` | Inicializa o Firebase Admin, cria a instância Nest, registra `ValidationPipe` global e abre a porta HTTP. | **Frameworks & Drivers** (Main) | *"O componente Main é o detalhe final (a política de nível mais baixo). É o ponto de entrada inicial do sistema."* |
| `app.module.ts` | Agrega todos os módulos de domínio em um único grafo de dependências. | **Frameworks & Drivers** (Composição) | Atua como "blocos reutilizáveis" de montagem. Não contém regra de negócio, apenas a ligação do framework. |
| `app.controller.ts` | Expõe `GET /` e `GET /protected`, enviando a lógica para o serviço e a autenticação para o `auth.guard.ts`. | **Adaptador de Interface** | Converte dados para o formato mais conveniente para o agente HTTP externo. |

### 5.1.2 Domínio `usuarios` (Padrão Builder)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `usuarios/models/usuario.ts` | Entidade-base com atributos e comportamentos comuns de um `Membro` (`autenticar`, `salvarTopico`). | **Entidade** | Regra de negócio pura, sem importações de framework: "as joias da família". |
| `usuarios/models/aluno.ts` / `admnistrador.ts` | Especializações contendo atributos e regras do domínio acadêmico da FCTE/UnB e de moderação. | **Entidade** | Herdam a pureza da entidade-base; representam puros conceitos de domínio. |
| `usuarios/builders/usuarioBuilder.ts` | Interface do padrão *Builder*, definindo o contrato de construção passo a passo. | **Entidade** (Contrato) | Define a forma como um objeto de domínio é montado: política sobre o próprio domínio. |
| `usuarios/builders/alunoBuilder.ts` / `admnistradorBuilder.ts` | Implementações concretas que constroem as entidades validando dados obrigatórios como matrícula. | **Caso de Uso** | Organizam a criação de entidades segundo uma política específica da aplicação. |
| `usuarios/usuarios.service.ts` | Controla os builders, chama o SDK do Firebase Admin e salva dados no Firestore com suporte a cancelamento (*rollback*). | **Caso de Uso** | Implementa a política de "registrar novo usuário", isolando o fluxo de dados do controller. |
| `usuarios/usuarios.controller.ts` | Expõe as rotas de criação de perfis, enviando o fluxo para o serviço de domínio. | **Adaptador de Interface** | Converte o corpo da requisição HTTP (DTO) em chamada ao caso de uso. |
| `usuarios/dtos/alunoDto.ts` / `admnistradorDto.ts` | Estruturas de validação de entrada baseadas nas regras de cadastro da plataforma. | **Adaptador de Interface** | DTOs são o formato conveniente para o agente externo consumir ou enviar dados. |
| `usuarios/usuarios.module.ts` | Declara providers e controllers do domínio para o módulo central. | **Frameworks & Drivers** | Mecanismo de injeção de dependência: detalhe do framework. |

### 5.1.3 Domínio `posts` (Padrão Factory Method)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `posts/interfaces/post.interface.ts` | Contrato que define as ações estáveis de uma publicação ou `Dica` de forma genérica. | **Entidade** | Define o comportamento essencial de "ser um post", sem dependências externas. |
| `posts/models/post-conteudo.model.ts` | Classe-base concreta com estado do post e regras de moderação da plataforma. | **Entidade** | Regra de negócio (quem pode fixar posts) independente do mecanismo de entrega. |
| Subclasses (`post-topico.model.ts`, `post-avaliacao.model.ts`, etc.) | Variações estruturais do domínio (Tópicos com fóruns, Avaliações com notas, Materiais com anexos). | **Entidade** | Cada subclasse guarda uma regra de negócio diferente sobre o mesmo conceito central. |
...
| Fábricas Concretas (`fabrica-topico.factory.ts`, etc.) | Implementações encarregadas de criar cada subtipo específico de postagem. | **Caso de Uso** | Coerente com o SRP: cada fábrica atende a um único fluxo de criação específico. |
| `posts/posts.service.ts` | Organiza as fábricas, gerencia o repositório em memória e junta as árvores de dados. | **Caso de Uso** | Centraliza a política "criar/listar posts", isolando o Controller da montagem dos dados. |
| `posts/posts.controller.ts` | Expõe endpoints protegidos por autenticação para criação e consumo de postagens e dicas. | **Adaptador de Interface** | Traduz a requisição HTTP autenticada em chamadas diretas ao serviço de Casos de Uso. |
| `posts/posts.module.ts` | Configura as ligações internas e injeções do pacote de postagens. | **Frameworks & Drivers** | Mecanismo de injeção de dependência: detalhe técnico do NestJS. |

### 5.1.4 Domínio `comentarios` e `blocos` (Padrão Composite)

| Componente / Pacote | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| Interfaces estruturais (`ComponenteBloco.ts`, etc.) | Contratos base do padrão Composite para tratamento igual de coleções e itens únicos. | **Entidade** | Abstração estrutural estável do domínio, independente da rede ou da interface. |
| Nós Folha (`comentario.model.ts`, `BlocoTexto.ts`, etc.) | Elementos individuais com regras isoladas de edição, denúncia ou formatação Markdown e LaTeX. | **Entidade** | Comportamento de negócio guardado individualmente, sem dependência de infraestrutura. |
| Nós Compostos (`thread-comentario.model.ts`, `BlocoLista.ts`) | Agregadores que processam coleções estruturadas e respostas de forma repetitiva (linhas de comentários). | **Entidade** | Implementa a regra estrutural de composição, reutilizável em qualquer cenário. |
...
| `blocos/blocos.controller.ts` | Converte chamadas HTTP de mudança de blocos em chamadas aos serviços. | **Adaptador de Interface** | Traduz as interações externas em chamadas direcionadas para os serviços do núcleo. |
| Módulos (`*.module.ts`) | Ligações e conexões do sistema de dependências do NestJS. | **Frameworks & Drivers** | Detalhes de infraestrutura associados ao framework de injeção escolhido. |

### 5.1.5 Domínio `observer` (Padrão Observer)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `observer/Observer.ts` / `Notificavel.ts` / `TipoEvento.ts` | Contratos e enums que definem o funcionamento de reatividade e avisos. | **Entidade** | Define a abstração de reação a eventos de domínio, sem dependência de transporte. |
| `observer/Usuario.ts` | Implementação concreta do observador (guarda as notificações recebidas no perfil do Membro). | **Entidade** | Regra de negócio pura: como um usuário reage a atualizações no sistema. |
| `observer/PostConteudo.ts` / `Comentario.ts` | Sujeitos concretos (Subjects) que ativam notificações em suas rotinas internas. | **Entidade** | Guardam a regra de quando notificar (ações de upvote/downvote ou novas respostas). |
| `observer/observer.service.ts` | Repositórios em memória; gerencia as ligações e o envio de alertas. | **Caso de Uso** | Política de aplicação "registrar interesse e notificar" (controla os fluxos do painel). |
| `observer/observer.controller.ts` | Expõe rotas REST para gerenciar observadores e testar notificações. | **Adaptador de Interface** | Tradução pura HTTP para Caso de Uso, sem regras de negócio próprias. |

### 5.1.6 Domínio `ordenacao-strategy` (Padrão Strategy)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| Contratos (`algoritmo-ordenacao.interface.ts`, etc.) | Abstração da regra de ordenação variável do feed e modelo do item ordenável. | **Entidade** | Abstração de uma regra variável, independente de como ou por quem é acionada. |
| Estratégias Concretas (`OrdenaPorVotos.ts`, `OrdenaPorData.ts`, etc.) | Algoritmos de organização simples, isolados e livres de efeitos colaterais de banco de dados. | **Entidade** | Fórmulas puras; a organização pelo tempo ($\text{relevância} = \frac{\text{votos}}{\text{horas} + 2}$) atua como regra isolada de domínio. |
| `ordenacao-strategy/Feed.ts` | Contexto do Strategy: guarda o algoritmo ativo e passa a tarefa de ordenação. | **Caso de Uso** | Aplica uma regra de domínio a uma lista conforme a necessidade do momento. |
...
| `ordenacao-strategy.controller.ts` / DTOs | Recebem os dados e parâmetros de organização vindos da internet. | **Adaptador de Interface** | Traduzem a entrada externa (JSON) em uma chamada limpa para o serviço. |

### 5.1.7 Domínio `proxy` (Padrão Proxy (Avaliação de Professores e Disciplinas))

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `proxy/Professor.ts` / `Avaliacao.ts` | Contrato comum e modelos puros que regem o sistema de avaliação baseado no SIGAA. | **Entidade** | Abstrações estáveis de negócio, independentes de caches ou origens de dados. |
| `proxy/ProfessorReal.ts` | Classe encarregada de fazer o cálculo de médias direto nas coleções. | **Entidade** | Regra de negócio (cálculo de matemática) isolada de qualquer otimização técnica. |
| `proxy/ProfessorProxy.ts` | Intercepta o objeto real, aplicando regras de cache invisível e controle de acessos. | **Caso de Uso** | Implementa uma política da aplicação (melhoria de acesso) e não uma regra natural do domínio. |
| `proxy/ProfessorService.ts` | Ponto de entrada do pacote; gerencia e organiza o ciclo de vida do par Real/Proxy. | **Caso de Uso** | Serve como o ponto único de acesso da aplicação a essa funcionalidade. |

### 5.1.8 Domínio `forum` (Padrão Facade)

| Arquivo | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `forum/forum.facade.ts` | Junta os serviços de vários pacotes em uma API simplificada para o fórum. | **Adaptador de Interface** | Funciona como um adaptador que converte dados dos casos de uso para o formato do agente externo. |
| `forum/forum.controller.ts` / DTOs | Expõe o fórum via HTTP, incluindo lógicas de diagnóstico e registro de logs. | **Adaptador de Interface** | Tradução HTTP para Facade. Mantém detalhes de infraestrutura (logs) fora do núcleo estável. |
...
| Arquivo / Componente | Função no Sistema | Camada | Fundamentação (Robert C. Martin) |
| --- | --- | --- | --- |
| `main.tsx` | Inicializa a árvore React e faz a montagem no ponto raiz do navegador (`#root`). | **Frameworks & Drivers** (Main) | É o ponto de entrada inicial do sistema: equivalente, no front-end, ao `main.ts` do servidor. |
| `firebase.ts` | Inicializa o sistema cliente do Firebase Authentication usando variáveis de ambiente. | **Frameworks & Drivers** | Detalhe técnico externo isolado da lógica, permitindo trocar a infraestrutura se necessário. |
| `utils/firebaseDebugToken.ts` | Grava e recupera dados de teste conversando com a API local (`localStorage`). | **Frameworks & Drivers** | Acesso direto a uma ferramenta nativa do navegador, sendo um detalhe que pode ser trocado. |
| `App.tsx` | Centraliza as rotas da interface do TenhoUmaDica usando o `react-router-dom`. | **Adaptador de Interface** (Humble Object) | Segue o padrão *Humble Object*: apenas junta rotas e dados, sem lógica interna, facilitando os testes. |
| Páginas (`PostPage.tsx`, `AvaliacoesPage.tsx`, etc.) | Fazem chamadas `fetch`, controlam subcomponentes e mostram dados na tela. | **Adaptador de Interface** (View/Presenter) | Converte as respostas JSON dos casos de uso em formatos visuais (papel clássico de Presenter). |
| `components/FirebaseAuthModal.tsx` | Modal utilitário que usa direto endpoints REST de teste do provedor. | **Adaptador de Interface** | Componente de borda; liga a interface direto a um detalhe externo para ajudar nos testes. |
